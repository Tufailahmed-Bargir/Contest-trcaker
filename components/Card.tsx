...
// Use the shared Contest type so the front-end accepts the same structure the API returns
type ContestProp = Contest;
...
export function ContestCard({
  ...
  const normalizedStatus = (contest.status as string)?.toLowerCase();
  const statusColors = {
  ...
  console.log('contest data been passed', contest);
    ...
    >
      <Card className="overflow-hidden h-full hover:shadow-lg transition-all duration-300 border-t-4 border-t-blue-500">
            ...
            <div className="flex items-center space-x-2">
              <span
                className={cn(
                  ...
                  // normalize to lowercase to match mapping keys
                  statusColors[(contest.status as string).toLowerCase() as keyof typeof statusColors],
                )}
              >
                {statusIcons[(contest.status as string).toLowerCase() as keyof typeof statusIcons]}
                {contest.status}
              </span>
            </div>
          </div>
        ...
        <CardFooter className="flex gap-2 pt-2 border-t">
          ...
          {normalizedStatus !== "upcoming" && contest.solution && (
  <Button
    variant="outline"
    size="sm"
